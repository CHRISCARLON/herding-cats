...
from HerdingCats.config.source_endpoints import CkanApiPaths
...
def test_ckan_health_check():
    ...
    with CatSession(CkanDataCatalogues.UK_GOV) as cat_session:
        ...
        try:
            ...
            if "success" in data:
                assert data["success"], (
                    f"CKAN returned success=False for {cat_session.base_url}"
                )
            ...
            pytest.fail(
                f"Failed to connect to CKAN endpoint for {cat_session.base_url}: {str(e)}"
            )
